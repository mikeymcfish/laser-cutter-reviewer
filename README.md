...
- Selecting an open-cut, crossing/touch, self-intersection, or coincident-overlap finding circles and numbers the exact problem locations in the normalized preview. The regression fixtures cover open paths, ordinary crossings, self-crossings, partial overlaps, and thin nested rings.
- SVG viewport CSS, selector specificity, definition containers, embedded-font ordering, and transformed raster resolution are resolved defensively before readiness is calculated. Automatic fixes are withheld whenever they would create a cut from a zero-width stroke or leave artwork outside a corrected artboard.
