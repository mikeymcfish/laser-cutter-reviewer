...
vi.mock('three', async (importOriginal) => {
  ...
  class WebGLRendererMock {
    ...
    setPixelRatio() {}
    setSize() {}
    render(scene: THREE.Scene) { threeState.scene = scene }
    dispose() {}
  }
...
describe('Preview3D embedded rasters', () => {
  ...
  it('projects only sanitized PNG layers onto cut-piece top surfaces and disposes their resources', () => {
    ...
    expect(screen.getByRole('img', { name: /with 1 embedded image layer on the piece surfaces/ })).toBeInTheDocument()
    ...
    expect(imageSurfaces).toHaveLength(1)
    expect(imageSurfaces[0].userData).toMatchObject({ previewRasterLayerId: 'photo', previewPieceId: 'piece' })
    const material = imageSurfaces[0].material as THREE.ShaderMaterial
    ...
    expect(imageSurfaces[0].position.z).toBeGreaterThan(3 + Math.min(0.16, 3 * 0.05))

    const texture = threeState.textures[0]
    const materialDispose = vi.spyOn(material, 'dispose')
    const geometryDispose = vi.spyOn(imageSurfaces[0].geometry, 'dispose')
    unmount()
